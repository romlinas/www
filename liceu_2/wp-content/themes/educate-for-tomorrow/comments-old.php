<?php // Do not delete these lines
	...
	if (!empty($post->post_password)) { // if there's a password
		...
		<p class="nocomments">У вас не достаточно прав. Введите пароль для просмотра комментариев.</p>
...
<?php if ($comments) : ?>
	...
	<?php foreach ($comments as $comment) : ?>
		...
		<li <?php echo $oddcomment; ?>id="comment-<?php comment_ID() ?>">
			...
			<cite><?php comment_author_link() ?></cite> Пишите:
			<?php if ($comment->comment_approved == '0') : ?>
			<em>Ваш комментарий ожидает модерации.</em>
			<?php endif; ?>
			<br />

			<small class="commentmetadata"><a href="#comment-<?php comment_ID() ?>" title=""><?php comment_date('F jS, Y') ?> в <?php comment_time() ?></a> <?php edit_comment_link('edit','&nbsp;&nbsp;',''); ?></small>
...
<label for="author"><small>Имя <?php if ($req) echo "(обязательные)"; ?></small></label></p>

<p><input type="text" name="email" id="email" value="<?php echo $comment_author_email; ?>" size="22" tabindex="2" <?php if ($req) echo "aria-required='true'"; ?> />
<label for="email"><small>Почта (не публикуется) <?php if ($req) echo "(обязательные)"; ?></small></label></p>
