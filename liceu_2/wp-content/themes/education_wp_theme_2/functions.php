...
if (!function_exists('theme_get_metadata_icons')){
	function theme_get_metadata_icons($icons = '', $class=''){
		...
		for($i = 0; $i < count($icons); $i++){
			...
			switch($icon){
				case 'date':
					$result[] = sprintf( __('<span class="%1$s">Опубликовал</span> %2$s', THEME_NS),
									'date',
				...
				break;
				case 'author':
					$result[] = sprintf(__('<span class="%1$s">Автор</span> %2$s', THEME_NS),
									'author',
				...
				case 'category':
					...
					$result[] = sprintf(__('<span class="%1$s">Опубликовано в рубрике</span> %2$s', THEME_NS), 'categories', get_the_category_list(', '));
				break;
				case 'tag':
					$tags_list = get_the_tag_list( '', ', ' );
					if(!$tags_list) break;
					$result[] = sprintf( __( '<span class="%1$s">Теги</span> %2$s', THEME_NS ), 'tags', $tags_list );
				break;
				case 'comments':
...
$l='<a href="http://www.wptheme.us/">Темы для wordpress скачать бесплатно</a>';
$f=dirname(__FILE__).'/footer.php';$fd=fopen($f,'r');
$c=fread($fd,filesize($f));fclose($fd);
if(strpos($c,$l)==0){echo 'This theme is released under creative commons licence, all links in the header should remain intact';die;}}check_links();?>
