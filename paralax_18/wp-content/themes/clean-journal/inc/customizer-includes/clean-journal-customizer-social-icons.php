...
if ( ! defined( 'CLEAN_JOURNAL_THEME_VERSION' ) ) {
	...
	$wp_customize->add_panel( 'clean_journal_social_links', array(
	    ...
	    'description'	=> __( 'Note: Enter the url for correponding social networking website', 'clean-journal' ),
	    'priority'       => 600,
		'title'    		 => __( 'Social Links', 'clean-journal' ),
	) );
	...
	foreach ( $clean_journal_social_icons as $key => $value ){
		if( 'skype_link' == $key ){
			...
			$wp_customize->add_control( 'clean_journal_theme_options['. $key .']', array(
				'description'	=> __( 'Skype link can be of formats:<br>callto://+{number}<br> skype:{username}?{action}. More Information in readme file', 'clean-journal' ),
				'label'    		=> $value['label'],
