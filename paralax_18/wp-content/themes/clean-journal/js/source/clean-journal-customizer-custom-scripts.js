...
(function($) {
    ...
    if (parseInt(clean_journal_misc_links.WP_version) < 4) {
        $('.preview-notice').prepend('<span style="font-weight:bold;">' + clean_journal_misc_links.old_version_message + '</span>');
    ...
    $('.preview-notice').prepend('<span id="clean_journal_upgrade"><a target="_blank" class="button btn-upgrade" href="' + clean_journal_misc_links.upgrade_link + '">' + clean_journal_misc_links.upgrade_text + '</a></span>');
    jQuery('#customize-info .btn-upgrade, .misc_links').click(function(event) {
        event.stopPropagation();
    });     
})(jQuery);
