...
function optionsframework_uploader( $_id, $_value, $_desc = '', $_name = '' ) {
	...
	$output .= '<input id="' . $id . '" class="upload' . $class . '" type="text" name="'.$name.'" value="' . $value . '" placeholder="' . __('Файл не выбран', 'options_framework_theme') .'" />' . "\n";
	if ( function_exists( 'wp_enqueue_media' ) ) {
		if ( ( $value == '' ) ) {
			$output .= '<input id="upload-' . $id . '" class="upload-button button" type="button" value="' . __( 'Загрузить', 'options_framework_theme' ) . '" />' . "\n";
		} else {
			$output .= '<input id="remove-' . $id . '" class="remove-file button" type="button" value="' . __( 'Удалить', 'options_framework_theme' ) . '" />' . "\n";
		}
	} else {
	...
	if ( $value != '' ) { 
		$remove = '<a class="remove-image">Удалить</a>';
...
function optionsframework_media_scripts( $hook ) {
	...
	wp_localize_script( 'of-media-uploader', 'optionsframework_l10n', array(
		'upload' => __( 'Загрузить', 'options_framework_theme' ),
		'remove' => __( 'Удалить', 'options_framework_theme' )
	) );
}
