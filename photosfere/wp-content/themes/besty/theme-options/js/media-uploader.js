(function($) {
	$(document).ready(function() {
		...
		function besty_add_file(event, selector) {
			...
			frame.on( 'select', function() {
				...
				selector.find('.remove-image, .remove-file').on('click', function() {
					besty_remove_file( $(this).parents('.section') );
				});
			});
		...
		function besty_remove_file(selector) {
			...
			selector.find('.upload-button').live('click', function() {
				besty_add_file(event, $(this).parents('.section'));
			});
		}
