<?php
// photographyThemes Admin Interface: Setup, Pages, Machine

function photographythemes_add_admin() {

    global $query_string;
    
    $options =  get_option('photography_template');      
    $themename =  get_option('photography_themename');      
    $shortname =  get_option('photography_shortname');      
    ...
    if ( $_GET['page'] == 'photographythemes_home' ||
         $_GET['page'] == 'photographythemes_advertising' ||
         $_GET['page'] == 'photographythemes_uploader' ||
         $_GET['page'] == 'photographythemes_image') 
    {
        ...
        if ( 'save' == $_REQUEST['action'] ) {
                ...
                foreach ($options as $key => $value) {
                    ...
                    $live_page = $_GET['page'];
                    $current_page = 'photographythemes_'.$value['page'];
                    
                    if($current_page == 'photographythemes_'){$current_page = 'photographythemes_home';}
           ...
           foreach ($options as $key => $value) {
                ...
                update_option('photography_upload_errors',$errors);
                
                //Create, Encrypt and Update the Saved Settings
                global $wpdb;
                $query = "SELECT * FROM $wpdb->options WHERE option_name LIKE 'photography_%' AND NOT option_name = 'photography_template' AND NOT option_name = 'photography_custom_template' AND NOT option_name = 'photography_settings_encode'";
                ...
                update_option('photography_settings_encode',$output);
                //End
            ...
            global $wpdb;
            $query = "DELETE FROM $wpdb->options WHERE option_name LIKE 'photography_%'";
            $wpdb->query($query);
...
// Check all the Options, then if the no options are created for a ralitive sub-page... it's not created.
    ...
    {
        add_object_page ('Page Title', 'Theme Options', 8,'photographythemes_home', 'photographythemes_page_gen', 'http://www.imithemes.com/favicon.png');
    }
    else
    {
        add_menu_page ('Page Title', 'Theme Options', 8,'photographythemes_home', 'photographythemes_page_gen', 'http://www.photographythemes.com/favicon.png'); 
    }

         if ($page_advertising){ add_submenu_page('photographythemes_home', $themename, 'Advertising', 8, 'photographythemes_advertising', 'photographythemes_advertising'); }
         if ($page_nav){ add_submenu_page('photographythemes_home', $themename, 'Navigation', 8, 'photographythemes_nav', 'photographythemes_nav'); }
         if ($page_resizing){  add_submenu_page('photographythemes_home', $themename, 'Image Resizing', 8, 'photographythemes_image', 'photographythemes_image'); }
...
function photographythemes_advertising(){ photographythemes_page_gen('advertising'); }
function photographythemes_nav(){ photographythemes_page_gen('nav'); }
function photographythemes_image(){ photographythemes_page_gen('image'); }

function photographythemes_page_gen($page){
 
    $options =  get_option('photography_template');      
    $themename =  get_option('photography_themename');      
    $shortname =  get_option('photography_shortname');
    $manualurl =  get_option('photography_manual'); 
...
// END
?>
<div class="wrap" id="photography_options">
    <form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post"  enctype="multipart/form-data">
        <div id="scrollme"><p class="submit"><input name="save" type="submit" value="Сохранить все изменения" /></p></div>
        ...
        <?php //Errors
        $upload_errors = get_option('photography_upload_errors');
        ...
        echo '<div style="clear:both;height:20px;"></div><div class="errors"><ul>' . "\n";
            ...
            foreach($upload_errors as $error)
            {
                 if($error['error'] == 'folder_not_created' && $error_shown != true){
                      echo '<li><strong>Oh No!</strong> We don\' have the permissions to create the upload folder on your server. Please create it manually: <em>/wp-content/<strong>photography_uploads</strong></em>. Thanks!</li>' . "\n";
                      $error_shown = true;
                }
        ...
        echo '</ul></div>' . "\n";
        delete_option('photography_upload_errors');
        } ?>

        <div style="clear:both;height:10px;"></div>
        <?php echo photographythemes_machine($options,$page);  //The real work horse  ?>
    ...
    <form action="<?php echo wp_specialchars( $_SERVER['REQUEST_URI'] ) ?>" method="post">
        ...
        <input name="reset" type="submit" value="Сбросить параметры" class="reset-button" onclick="return confirm('Click OK to reset. Any settings will be lost!');" />
...
function photographythemes_machine($options,$page) {
    ...
    foreach ($options as $value) {
        ...
        case "multicheck":
            ...
            $photography_key = $value['id'] . '_' . $key;
            $saved_std = get_option($photography_key);
            ...
            $output .= '<input type="checkbox" class="checkbox" name="'. $photography_key .'" id="'. $photography_key .'" value="true" '. $checked .' /><label for="'. $photography_key .'">'. $option .'</label><br />';
        ...
        case "upload":
            ...
            $output .= photographythemes_uploader_function($value['id'],$value['std'],'options');
        ...
        case "heading":
            ...
            $output .= '<h3><span class="photography-expand">+</span>'. $value['name'] .'</h3>'."\n";    
            $output .= '</div>'."\n";
...
// photographyThemes Uploader
function photographythemes_uploader_function($id,$std){

    $uploader .= '<input type="file" name="attachement_'.$id.'" class="upload_input"></input>';
    $uploader .= '<span class="submit"><input name="save" type="submit" value="Загрузка" class="button upload_save" /></span>';
...
<script type="text/javascript">
    jQuery(document).ready(function(){
        ...
        jQuery('#photography_options .title h3').parent().next('.option_content').slideUp();
        
        var initChar;
        jQuery('#photography_options .title h3').hover(function(){
        ...
        jQuery('#photography_options .title h3').click(function(){
            if (jQuery(this).parent().next('.option_content').css('display') == 'none'){
