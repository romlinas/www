...
function photography_get_image($key = 'image', $width = null, $height = null, $class = "thumbnail", $quality = 90,$id = null,$link = 'src',$repeat = 1,$offset = 0,$before = '', $after = '',$single = false, $force = false, $return = false) {
    ...
    if(!empty($custom_field)) { // If the user set a custom field
        ...
        $force_all = get_option('photography_force_all');
        $force_single = get_option('photography_force_single');
        if($force == true OR $force_all == true OR ($force_single == true AND is_single())){  
                $set_width = '';
                $set_height = '';
        }
    
        if (get_option('photography_resize') == 'true') { 
    ...
    elseif(empty($custom_field) && get_option('photography_auto_img') == 'true'){
