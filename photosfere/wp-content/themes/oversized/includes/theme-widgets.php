...
<div id="flickr" class="widget">
	...
	<h3 class="widget_title"><?php _e('Flickr Photos',photographythemes); ?><span class="fold">&nbsp;</span></h3>
...
register_sidebar_widget('photography - Flickr', 'flickrWidget');

register_widget_control('photography - Flickr', 'flickrWidgetAdmin', 400, 200);
...
if (get_option('photography_ads_rotate') == 'true') {
...
<?php
	...
	foreach ($numbers as $number) {	
		...
		$img_url[$counter] = get_option('photography_ad_image_'.$number);

		$dest_url[$counter] = get_option('photography_ad_url_'.$number);
...
register_sidebar_widget('photography - Ads 305x240', 'adsWidget');
...
register_widget_control('photography - Ads 305x240', 'adsWidgetAdmin', 200, 200);
...
function widget_Twidget_init() {
	...
	function widget_Twidget($args) {
		...
		echo '<span class="wephotographyte"><a href="http://www.twitter.com/'.$account.'/" title="Follow us on Twitter" target="_blank"><img src="http://www.negrita.com/new2011/wp-content/uploads/2011/09/follow-us-on-twitter.jpg" alt="" /></a></span></div>';
	...
	function widget_Twidget_control() {
		...
		if ( !is_array($options) )
			...
			$options = array('account'=>'photographythemes', 'title'=>'Twitter Updates', 'show'=>'3');
	...
	register_sidebar_widget(array('photography - Twitter', 'widgets'), 'widget_Twidget');



	// Register settings for use, 300x200 pixel form

	register_widget_control(array('photography - Twitter', 'widgets'), 'widget_Twidget_control', 300, 200);
...
register_sidebar_widget('photography - Search', 'SearchWidget');
...
function photography_deregister_widgets(){

    unregister_widget('WP_Widget_Search');         

}

add_action('widgets_init', 'photography_deregister_widgets');  
