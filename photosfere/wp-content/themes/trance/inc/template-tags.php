...
function trance_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( '%s', 'post date', 'trance' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( '%s', 'post author', 'trance' ),
...
function trance_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		if ( $categories_list && trance_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( '%1$s', 'trance' ) . '</span>', $categories_list ); // WPCS: XSS OK
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'trance' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( '%1$s', 'trance' ) . '</span>', $tags_list ); // WPCS: XSS OK
		}
	}
