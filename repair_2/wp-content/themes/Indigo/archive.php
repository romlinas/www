...
<div class="span-24" id="contentwrap">
	<div class="span-16">
 	  ...
 	  <?php /* If this is a daily archive */ } elseif (is_day()) { ?>
		<h2 class="pagetitle">Архив <?php the_time('j F Y'); ?></h2>
	  ...
	  <?php /* If this is an author archive */ } elseif (is_author()) { ?>
 	  ...
 	  <?php } ?>
		...
		<div <?php post_class() ?>>
				...
				<div class="postmeta"><img src="<?php bloginfo('template_url'); ?>/images/folder.png" /> Рубрика: <?php the_category(', ') ?> <?php if(get_the_tags()) { ?> <img src="<?php bloginfo('template_url'); ?>/images/tag.png" /> <?php  the_tags('Метки: ', ', '); } ?>  <img src="<?php bloginfo('template_url'); ?>/images/comments.png" /> <?php comments_popup_link('Нет комментариев  &#187;', '1 Комментарий  &#187;', 'Комментариев: % &#187;'); ?></div>
	...
	<?php else :
		...
		if ( is_category() ) { // If this is a category archive
			printf("<h2 class='pagetitle'>Извените, но здесь не существует каких-либо записей в: %s</h2>", single_cat_title('',false));
		} else if ( is_date() ) { // If this is a date archive
			echo("<h2 class='pagetitle'>Извините, но записей с этой датой не существует.</h2>");
		} else if ( is_author() ) { // If this is a category archive
			$userdata = get_userdatabylogin(get_query_var('author_name'));
			printf("<h2 class='pagetitle'>Извените, но не существует каких-либо записей для %s</h2>", $userdata->display_name);
		} else {
			echo("<h2 class='pagetitle'>Записи не найдены.</h2>");
		}
