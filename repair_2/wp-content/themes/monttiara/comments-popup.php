...
<head>
     <title><?php echo get_settings('blogname'); ?> - <?php echo sprintf(__("Отзывы на %s"), the_title('','',false)); ?></title>
...
<h2 id="comments"><?php _e("Обсуждение"); ?></h2>

<p><a href="<?php echo get_settings('siteurl'); ?>/wp-commentsrss2.php?p=<?php echo $post->ID; ?>"><?php _e("RSS-лента комментариев."); ?></a></p>

<?php if ('open' == $post->ping_status) { ?>
<p><?php _e("Адрес для трекбека:"); ?> <em><?php trackback_url() ?></em></p>
<?php } ?>
...
<?php foreach ($comments as $comment) { ?>
	...
	<?php comment_text() ?>
	<p><cite><?php comment_type(__('Отзыв'), __('Трекбек'), __('Пинг')); ?>, <?php comment_author_link() ?> &#8212; <?php comment_date() ?> в <a href="#comment-<?php comment_ID() ?>"><?php comment_time() ?></a></cite></p>
	</li>

<?php } // end for each comment ?>
</ol>
<?php } else { // this is displayed if there are no comments so far ?>
	<p><?php _e("Отзывов пока нет."); ?></p>
<?php } ?>
...
<form action="<?php echo get_settings('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
	...
	<p>
	  <label for="comment"><?php _e("Сообщение"); ?></label>
	<br />
