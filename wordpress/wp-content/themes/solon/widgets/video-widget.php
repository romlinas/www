...
// constructor
	...
	<p><label for="<?php echo $this->get_field_id( 'url' ); ?>"><?php _e( 'Paste the URL of the video (only from a network that supports oEmbed, like Youtube, Vimeo etc.):', 'solon' ); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['solon_video_widget']) )
			delete_option('solon_video_widget');		  
		  
		return $instance;
	}
	
	function flush_widget_cache() {
		wp_cache_delete('solon_video_widget', 'widget');
	}
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'solon_video_widget', 'widget' );
		}
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Video', 'solon' );
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'solon_video_widget', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
