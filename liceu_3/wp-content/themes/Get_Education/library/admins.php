...
<div class="wrap">
	...
	<h2><?php _e('Настройки темы', THEME_NS); ?></h2>
<?php 
	if ( isset($_REQUEST['Submit']) )	{ 
		...
		echo '<div id="message" class="updated fade"><p><strong>'. __('Настройки сохранены.', THEME_NS) .'</strong></p></div>'."\n"; 
	} 
	if ( isset($_REQUEST['Reset']) )	{ 
	foreach ($theme_options as $value) {
		delete_option(theme_get_array_value($value, 'id')); 
	}
	echo '<div id="message" class="updated fade"><p><strong>'. __('Настройки сброшены.', THEME_NS) . '</strong></p></div>'."\n";
	} 
...
?>
	<p class="submit">
		<input name="Submit" type="submit" class="button-primary" value="<?php echo esc_attr(__('Сохранить', THEME_NS)) ?>" />
		<input name="Reset" type="submit" class="button-secondary" value="<?php echo esc_attr(__('Сбросить', THEME_NS)) ?>" />
	</p>
	</form>
	</div>
<?php
