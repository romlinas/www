	<?php
		...
		if (!empty( $post->post_parent ) ) {
			$return_link = '<a href="'. get_permalink( $post->post_parent ) .'" title="'.esc_attr(sprintf( __('Вернуться на %s', THEME_NS), get_the_title($post->post_parent))) .'" rel="gallery">'
		...
		if ( wp_attachment_is_image() ) {
					...
					echo '<p class="center">' . sprintf( __( 'Полный размер в пикселях %s', THEME_NS),
						sprintf( '<a href="%1$s" title="%2$s">%3$s &times; %4$s</a>',
							wp_get_attachment_url(),
							esc_attr( __('Ссылка на полноразмерное изображение', THEME_NS) ),
							$metadata['width'],
							$metadata['height']
						)
					) . '</p>';
