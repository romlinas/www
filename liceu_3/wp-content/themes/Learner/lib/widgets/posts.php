...
class ThematerPosts extends WP_Widget 
{
    ...
    {
        $widget_options = array('description' => 'Расширенный виджет для отображения последних записей или записей из выбранных рубрик или меток.');
        $control_options = array( 'width' => 400);
		$this->WP_Widget('themater_posts', '&raquo; Записи с каринками', $widget_options, $control_options);
    }
                            ...
                            if($instance['display_content'] || $instance['display_read_more']) {
                                ?><div class="posts-widget-entry"><?php 
                                    ...
                                    if($instance['display_read_more']) {
                                        ?> <a class="posts-widget-more" href="<?php the_permalink() ?>" rel="bookmark" title="Постоянная ссылка для <?php the_title_attribute(); ?>">Читать далее</a><?php 
                                    }?>
                                </div><?php
                            }
                        ?>
    ...
    {	
        ...
        <div class="tt-widget">
            <table width="100%">
                <tr>
                    <td class="tt-widget-label" width="25%"><label for="<?php echo $this->get_field_id('title'); ?>">Заголовок:</label></td>
                ...
                <tr>
                    <td class="tt-widget-label"><label for="<?php echo $this->get_field_id('posts_number'); ?>">Количество записей:</label></td>
                ...
                <tr>
                    <td class="tt-widget-label"><label for="<?php echo $this->get_field_id('order_by'); ?>">Сортировка записей:</label></td>
                    <td class="tt-widget-content">
                        <select id="<?php echo $this->get_field_id('order_by'); ?>" name="<?php echo $this->get_field_name('order_by'); ?>">
                            <option value="none" <?php selected('none', $instance['order_by']); ?> >Нет (По умолчанию)</option>
                            <option value="id_asc" <?php selected('id_asc', $instance['order_by']); ?> >ID ( По возрастанию ) </option>
                            <option value="id_desc" <?php selected('id_desc', $instance['order_by']); ?> >ID ( По убыванию ) </option>
                            <option value="date_asc"  <?php selected('date_asc', $instance['order_by']); ?>>Дата ( По возрастанию ) </option>
                            <option value="date_desc"  <?php selected('date_desc', $instance['order_by']); ?>>Дата ( По убыванию ) </option>
                            <option value="title_asc" <?php selected('title_asc', $instance['order_by']); ?>>Заголовок ( По возрастанию ) </option>
                            <option value="title_desc" <?php selected('title_desc', $instance['order_by']); ?>>Заголовок ( По убыванию  ) </option>
                            <option value="rand" <?php selected('rand', $instance['order_by']); ?>>Случайно</option>
                        </select>
                    </td>
                </tr>
                
                <tr>
                    <td class="tt-widget-label">Отображаемые элементы:</td>
                    <td class="tt-widget-content">
                        <input type="checkbox" name="<?php echo $this->get_field_name('display_title'); ?>"  <?php checked('true', $instance['display_title']); ?> value="true" />  Заголовок записи
                        ...
                        <br /><input type="checkbox" name="<?php echo $this->get_field_name('display_content'); ?>"  <?php checked('true', $instance['display_content']); ?> value="true" />  Содержимое / Цитата
                        ...
                        <br /><input type="checkbox" name="<?php echo $this->get_field_name('display_read_more'); ?>"  <?php checked('true', $instance['display_read_more']); ?> value="true" />  Ссылка "Читать далее"
                    </td>
                </tr>
                
                <tr>
                    <td class="tt-widget-label">Тип содержимого:</td>
                    <td class="tt-widget-content">
                        <input type="radio" name="<?php echo $this->get_field_name('content_type'); ?>" <?php checked('the_content', $instance['content_type']); ?> value="the_content" /> Контент<br />
                        <input type="radio" name="<?php echo $this->get_field_name('content_type'); ?>" <?php checked('the_excerpt', $instance['content_type']); ?> value="the_excerpt" /> Цитата &nbsp; <label for="<?php echo $this->get_field_id('excerpt_length'); ?>">Длина цитаты:</label> <input style="width: 40px;" id="<?php echo $this->get_field_id('excerpt_length'); ?>" name="<?php echo $this->get_field_name('excerpt_length'); ?>" type="text" value="<?php echo esc_attr($instance['excerpt_length']); ?>" /> <span class="tt-widget-help">слов(о/а)</span>
                    </td>
                </tr>
                
                <tr>
                    <td class="tt-widget-label">Миниатюра:</td>
                    <td class="tt-widget-content">
                        Шир.: <input type="text" style="width: 40px;" name="<?php echo $this->get_field_name('featured_image_width'); ?>" value="<?php echo esc_attr($instance['featured_image_width']); ?>" /> &nbsp; Выс.: <input type="text" style="width: 40px;" name="<?php echo $this->get_field_name('featured_image_height'); ?>" value="<?php echo esc_attr($instance['featured_image_height']); ?>"  />  
                         &nbsp; Выравн.: <select name="<?php echo $this->get_field_name('featured_image_align'); ?>">
                ...
                <tr>
                    ...
                    <td class="tt-widget-content" style="padding-top: 5px;">
                        <input type="radio" name="<?php echo $this->get_field_name('filter'); ?>" <?php checked('recent', $instance['filter']); ?> value="recent" /> Показывать новые записи <br /><br />
                       
                        <input type="radio" name="<?php echo $this->get_field_name('filter'); ?>" <?php checked('category', $instance['filter']); ?> value="category" /> Показывать записи из одной рубрики:<br />
                        ...
                        <input type="radio" name="<?php echo $this->get_field_name('filter'); ?>" <?php checked('cats', $instance['filter']); ?> value="cats" /> <label for="<?php echo $this->get_field_id('filter_cats'); ?>">Показывать записи из выбранных рубрик:</label>
                        <br /><span class="tt-widget-help">Идентификаторы рубрик ( например: 5,9,24 )</span>
                        ...
                        <br /><br /><input type="radio" name="<?php echo $this->get_field_name('filter'); ?>" <?php checked('tags', $instance['filter']); ?> value="tags" /> <label for="<?php echo $this->get_field_id('filter_tags'); ?>">Показывать только записи с метками:</label>
                        <br /><span class="tt-widget-help">Слаги меток ( например: computer,news,business-news )</span>
