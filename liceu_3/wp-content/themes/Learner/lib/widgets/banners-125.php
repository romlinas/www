...
class ThematerBanners125 extends WP_Widget 
{
    ...
    {
        $widget_options = array('description' => 'Баннеры размера 125x125 px.');
        $control_options = array( 'width' => 600);
		$this->WP_Widget('themater_banners_125', '&raquo; Баннеры 125x125', $widget_options,$control_options);
    }
    ...
    {	
        ...
        <div style="margin-bottom: 20px;">
            <a class="button" onclick="themater_125_banner_new();" >Добавить новый баннер</a> &nbsp; &nbsp; <input type="checkbox" name="<?php echo $this->get_field_name('randomize'); ?>" <?php checked('true', $instance['randomize']); ?> value="true" /> Показывать баннеры в случайном порядке
        </div>
        <?php
            if(is_array($get_banners)) {
                foreach($get_banners as $banner_id=>$banner_source) {
                    ...
                    <div class="tt-clearfix" style="padding: 0 0 20px 0; border-bottom: 1px solid #ddd; margin-bottom: 20px;" id="container_<?php echo $this->get_field_id($banner_id); ?>">
                        <div style="width: 133px; float: right; ">
                            ...
                            <div><a class="button"  onclick="themater_125_banner_preview('<?php echo $this->get_field_id($banner_id); ?>');">Показ.</a> <a class="button tt-button-red" onclick="if (confirm('Выбранный баннер будет удален! Вы действительно хотите продолжить?')) { themater_125_banner_delete('<?php echo $this->get_field_id($banner_id); ?>'); } return false;">Удалить</a></div>
                        </div>
                ...
                <div class="themater_125_banner_prototype" style="display: none;">
                    <div class="tt-clearfix" style="padding: 0 0 20px 0; border-bottom: 1px solid #ddd; margin-bottom: 20px;" id="container_the__id__">
                        <div style="width: 133px; float: right; ">
                            ...
                            <div><a class="button"  onclick="themater_125_banner_preview('the__id__');">Показ.</a> <a class="button tt-button-red" onclick="if (confirm('Выбранный баннер будет удален! Вы действительно хотите продолжить?')) { themater_125_banner_delete('the__id__'); } return false;">Удалить</a></div>
                        </div>
                        <div style="margin-right: 150px;">
