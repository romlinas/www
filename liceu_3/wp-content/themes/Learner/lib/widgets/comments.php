...
class ThematerComments extends WP_Widget 
{
    ...
    {
        $widget_options = array('description' => 'Расширенный виджет для отображения последних комментариев с аватарами');
        $control_options = array( 'width' => 400);
		$this->WP_Widget('themater_comments', '&raquo; Комментарии с аватарами', $widget_options, $control_options);
    }
                        ...
                        <li class="clearfix">
                            <?php 
                                ...
                                if( $instance['display_avatar']) { ?>
                                    <a href="<?php echo $get_the_peralink; ?>" title="<?php echo $comment->post_title; ?>"><img class="comments-widget-avatar <?php echo $instance['avatar_align']; ?>" src="http://www.gravatar.com/avatar.php?gravatar_id=<?php echo md5($comment->comment_author_email); ?>&amp;size=<?php echo $instance['avatar_size']; ?>" /></a><?php 
                                } 
    ...
    {	
        ...
        ?>
            ...
            <div class="tt-widget">
                <table width="100%">
                    <tr>
                        <td class="tt-widget-label" width="40%"><label for="<?php echo $this->get_field_id('title'); ?>">Заголовок:</label></td>
                    ...
                    <tr>
                        <td class="tt-widget-label"><label for="<?php echo $this->get_field_id('comments_number'); ?>">Количество комментариев:</label></td>
                    ...
                    <tr>
                        <td class="tt-widget-label"><label for="<?php echo $this->get_field_id('comment_length'); ?>">Длина комментария:</label></td>
                        <td class="tt-widget-content">
                            ...
                            <br /><span class="tt-widget-help">Количество слов</span>
                        </td>
                    </tr>
                    
                    <tr>
                        <td class="tt-widget-label"><label for="<?php echo $this->get_field_id('read_more_text'); ?>">Текст "Читать далее":</label></td>
                    ...
                    <tr>
                        <td class="tt-widget-label">Показываемые элементы:</td>
                        <td class="tt-widget-content">
                            <input type="checkbox" name="<?php echo $this->get_field_name('display_author'); ?>"  <?php checked('true', $instance['display_author']); ?> value="true" />  Автор
                            <br /><input type="checkbox" name="<?php echo $this->get_field_name('display_comment'); ?>"  <?php checked('true', $instance['display_comment']); ?> value="true" />  Комментарий
                            <br /><input type="checkbox" name="<?php echo $this->get_field_name('display_avatar'); ?>"  <?php checked('true', $instance['display_avatar']); ?> value="true" />  Аватар
                        </td>
                    </tr>
                    
                    <tr>
                        <td class="tt-widget-label">Аватар:</td>
                        <td class="tt-widget-content">
                            Размер: <input type="text" style="width: 40px;" name="<?php echo $this->get_field_name('avatar_size'); ?>" value="<?php echo esc_attr($instance['avatar_size']); ?>" />
                          Выравн.: <select name="<?php echo $this->get_field_name('avatar_align'); ?>">
