...
$themater_infobox_defaults = array(
    'title' => 'Об авторе',
    'description' => 'Добавить пару слов о себе.',
    'link' => '',
    'more_label' => 'Подробнее &raquo;',
    'image' => '',
...
class ThematerInfoBox extends WP_Widget 
{
    ...
    {
        $widget_options = array('description' => 'Выделите продукт или услугу с пользовательским изображением.');
        $control_options = array( 'width' => 480);
		$this->WP_Widget('themater_infobox', '&raquo; Блок "Об авторе"', $widget_options, $control_options);
    }
    ...
    {	
        ...
        <div class="tt-widget">
            <table width="100%">
                ...
                <tr>
                    <td class="tt-widget-label" width="25%"><label for="<?php echo $this->get_field_id('image'); ?>">Ссылка на картинку:</label></td>
                ...
                <tr>
                    <td class="tt-widget-label"><label for="<?php echo $this->get_field_id('description'); ?>">Описание:</label></td>
                ...
                <tr>
                    <td class="tt-widget-label" width="25%"><label for="<?php echo $this->get_field_id('link'); ?>">Ссылка:</label></td>
                ...
                <tr>
                    <td class="tt-widget-label" width="25%"><label for="<?php echo $this->get_field_id('more_label'); ?>">Метка "Читать далее":</label></td>
                ...
                <tr>
                    <td class="tt-widget-label">Настройка картинки:</td>
                    <td class="tt-widget-content">
                        Позиция: <select name="<?php echo $this->get_field_name('image_position'); ?>">
                            <option value="before_title" <?php selected('before_title', $instance['image_position']); ?> >Перед заголовком</option>
                            <option value="before_description"  <?php selected('before_description', $instance['image_position']); ?>>Перед описанием</option>
                            <option value="bottom" <?php selected('bottom', $instance['image_position']); ?>>Внизу</option>
                        </select>
                        
                         &nbsp; Выравн.: <select name="<?php echo $this->get_field_name('image_align'); ?>">
                ...
                <tr>
                    <td class="tt-widget-label">Добавить ссылки на:</td>
                    <td class="tt-widget-content">
                        <input type="checkbox" name="<?php echo $this->get_field_name('link_title'); ?>"  <?php checked('true', $instance['link_title']); ?> value="true" />  Заголовок
                        <br /><input type="checkbox" name="<?php echo $this->get_field_name('link_description'); ?>"  <?php checked('true', $instance['link_description']); ?> value="true" /> Описание
                        <br /><input type="checkbox" name="<?php echo $this->get_field_name('link_image'); ?>"  <?php checked('true', $instance['link_image']); ?> value="true" />  Картинка
                    </td>
                </tr>
