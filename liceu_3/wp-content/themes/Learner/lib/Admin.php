...
class ThematerAdmin
{
    ...
    function loadMenu()
	{
	   add_theme_page("Настройки темы ". $this->theme->theme_name, "Настройки темы ". $this->theme->theme_name, 'administrator', 'themater',  array(&$this, 'ThematerThemeOptions'));
	}
    ...
    function Ajax() 
	{
		...
		if (is_callable(array(get_class($this), $act))) {
            ...
            echo 'Вызов неопределенной функции ajax: ' . $act;
        }
		exit();
	}
    ...
    function ajax_savechanges()
    {
        ...
        echo '<span style="color:green; font-weight:bold;">Настройки сохранены!</span>';
    }
    ...
    function ajax_imageupload()
    {
        ...
        if(!in_array(strtolower($filename_ext),$allowed_imagetypes)) {
            echo 'Ошибка загрузки: расширение файла ' . $filename_ext . ' не разрешено!'; 
        } else {
            ...
            if(!empty($uploaded_image['error'])) {
                echo 'Ошибка загрузки: ' . $uploaded_image['error']; 
            } else { 
    ...
    function ajax_admin_options()
    {
        ...
        if($do == 'reset') {
            ...
            echo '<div class="tt-success">Настройки были сброшены. <a href="'. admin_url() . 'themes.php?page=themater">Кликните сюда</a>, чтобы обновить страницу настроек.</div>';
        }
    }
    ...
    function form_imageupload ($name, $attributes = array())
    {
        ...
        <div id="<?php echo $name; ?>_preview" style="<?php if(!$current_image) { ?>display: none;<?php }?>" class="tt-image-preview" ><?php if($current_image) { ?><a href="<?php echo $current_image; ?>" target="_blank"><img src="<?php echo $current_image; ?>" title="Изображение может быть изменено, нажмите для полного просмотра!" alt=""  /></a><br /><span>Изображение может быть изменено, нажмите для полного просмотра.</span><?php } ?></div>
        ...
        <div style="padding-bottom:10px;">
            <a id="themater_image_upload_<?php echo $name; ?>" class="tt_imageupload button" >Загрузить сейчас</a>
            <a <?php if(!$current_image) { ?> style="display: none;" <?php }?> id="<?php echo $name; ?>_reset" title="<?php echo $name; ?>" class="button tt_imageupload_reset" >Удалить</a>
        </div>
            ...
            <h2>Настройки темы | 
                <span class="tt-themename">
                    <?php 
                        ...
                        if($theme_url) {
                            printf("%s" . $this->theme->theme_name . " $theme_version%s", "<a href=\"$theme_url\" target=\"_blank\" title=\"Перейти на домашнюю страницу\">", "</a>");
                        } else {
                ...
                <div class="tt-container">
                    ...
                    <?php 
                            ...
                            <div style="clear: both; margin: 15px 0 0 0; padding: 15px;text-align: center; background: #ececec; border-top: 1px solid #d1d1d1;">
                                <a class="button-primary" onclick="themater_savechanges('savechanges','ttForm','ttSaveChanges');">Сохранить настройки</a>
                        ...
                        ?>
                                <div class="tt-form-element">
                                    <?php if($item_display == 'inline') {
                                        ...
                                        if($item_help) { ?>
                                                <a href="javascript:themater_showHide('<?php echo $itemval['name'] . '_help'; ?>');"><img src="<?php echo THEMATER_ADMIN_URL;?>/images/help.gif" class="tt-help" title="Кликните, чтобы получить подсказки" /></a>
                                        <?php }?>
