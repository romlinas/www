    ...
    function themater_loading(appendto) 
    {
    	...
    	$thematerjQ("#"+appendto+"").append('<p style="padding:4px; margin:4px;"><img src="images/loading.gif" align="absmiddle"> <span style="font-size:11px; color: #999;">Подождите...</span></p>');
    }
    ...
    function themater_savechanges(requests,formname,appendto) 
    {
        ...
        $thematerjQ("#"+appendto+"").empty();
        $thematerjQ("#"+appendto+"").show();
	    $thematerjQ("#"+appendto+"").append('<img src="images/loading.gif" align="absmiddle"> <span style="font-size:11px; color: #999;">Сохранение...</span>');
        return false;
    }
			...
			new AjaxUpload(clickedID, {
			  ...
			  onComplete: function(file, response) {
			   if(response.search('Upload Error') > -1){
                    ...
                    var previewImage = '<a href="'+response+'" target="_blank"><img src="'+response+'" title="Картинка могла измениться, кликните, чтобы просмотреть!" alt="" /></a><br /><span>Картинка могла измениться, кликните, чтобы просмотреть!</span>';
